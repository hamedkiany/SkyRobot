...
void configADC_IniciaADC(void)
{
    ...
    ADCSequenceConfigure(ADC0_BASE, 0, ADC_TRIGGER_PROCESSOR, 1); //Disparo software (processor trigger)
    ADCSequenceStepConfigure(ADC0_BASE, 0, 0, ADC_CTL_CH0);
    ADCSequenceStepConfigure(ADC0_BASE, 0, 1, ADC_CTL_CH1);
    ADCSequenceStepConfigure(ADC0_BASE, 0, 2, ADC_CTL_CH2);
    ADCSequenceStepConfigure(ADC0_BASE, 0, 3, ADC_CTL_CH3); //La ultima muestra provoca la interrupcion
    ADCSequenceStepConfigure(ADC0_BASE, 0, 4, ADC_CTL_CH4); //Disparo software (processor trigger)
...
void configADC_Timer(void)
{
   SysCtlPeripheralEnable(SYSCTL_PERIPH_ADC0);
    ...
    ADC_CTL_CH7 | ADC_CTL_IE | ADC_CTL_END); //La ultima muestra provoca la interrupcion
    ADCSequenceEnable(ADC0_BASE, 0); //ACTIVO LA SECUENCIA
    ADCHardwareOversampleConfigure(ADC0_BASE,64);//eso es para over sampling
